...
ssize_t my_read(struct file *file, char __user *ubuff, size_t cnt, loff_t *offset)
	{
		...
		wait_event_interruptible(my_waitq,CIRC_CNT(my_buff.head,my_buff.tail,SIZE)>0);
...
//		printk("before write head   %d   tail %d",my_buff.head,my_buff.tail);
		printk("SPACE AVILABLE : %d\n",CIRC_SPACE(my_buff.head,my_buff.tail,SIZE));
		 wait_event_interruptible(my_waitq,CIRC_SPACE(my_buff.head,my_buff.tail,SIZE)>1);
		 printk("test\n");

		k=min((int)cnt,CIRC_SPACE(my_buff.head,my_buff.tail,SIZE)); //
		for(i=0;i<k;i++)
		{printk("HEAD    ;%d \n",my_buff.head);
